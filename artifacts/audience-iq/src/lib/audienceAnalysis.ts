...
const ALL_REGIONS = [
  ...
  { region: 'Kuwait',        lat: 29.4, lng:  47.9 },
  { region: 'Tunisia',       lat: 33.9, lng:   9.5 },
  { region: 'Lebanon',       lat: 33.9, lng:  35.5 },
  { region: 'Iraq',          lat: 33.2, lng:  43.7 },
  { region: 'Bahrain',       lat: 26.0, lng:  50.5 },
  { region: 'Oman',          lat: 21.5, lng:  55.9 },
  { region: 'Libya',         lat: 26.3, lng:  17.2 },
  { region: 'Algeria',       lat: 28.0, lng:   1.7 },
  { region: 'Sudan',         lat: 12.9, lng:  30.2 },
  { region: 'Yemen',         lat: 15.6, lng:  48.5 },
  { region: 'Palestine',     lat: 31.9, lng:  35.2 },
  { region: 'Syria',         lat: 34.8, lng:  38.9 },
  { region: 'Mauritania',    lat: 21.0, lng: -10.9 },
...
function buildGeoRegions(type: ContentType, goal: Goal, s: number, hasArabic: boolean): GeoRegion[] {
  ...
  const scored = ALL_REGIONS.map((rd, idx) => {
    ...
    if (hasArabic) {
      const arabCountries = ['Saudi Arabia','UAE','Qatar','Kuwait','Bahrain','Oman','Egypt','Jordan','Lebanon','Syria','Iraq','Palestine','Morocco','Algeria','Tunisia','Libya','Sudan','Mauritania','Somalia','Yemen'];
      if (arabCountries.includes(rd.region)) reach = Math.min(95, Math.round(reach * 1.14));
    }
...
const GAP_POOLS: GapMap = {
  ...
  education: {
    ...
    applicants: [{ id:'g1',icon:'🎓',problem:'No student success story',action:'Add a short alumni quote in the first 3 lines',potentialGain:40 },{ id:'g2',icon:'🔗',problem:'Enrollment CTA is at the bottom',action:'Move the application link to the second line',potentialGain:32 },{ id:'g3',icon:'🌏',problem:'Egypt & Jordann audience not targeted',action:'Schedule the post at IST 8–10 AM on Tuesday',potentialGain:26 }],
  },
  recruitment: {
...
const MISSED_AUD: GrowthMap = {
  ...
  tech: {
    ...
    applicants: [{ icon:'💻',iconBg:'bg-green-100',title:'Senior Engineers (5+ yrs)',reason:'No senior-level challenge mentioned',action:'Add "Complex problems, real product ownership" to post',gain:'High Potential' },{ icon:'🌍',iconBg:'bg-rose-100',title:'Global Developer Communities',reason:'Not posted in developer forums',action:'Cross-post to Arab Developers and /r/cscareerquestions',gain:'Medium Potential' }],
  },
  general: {
...
const MISSED_REG: GrowthMap = {
  food: {
    viewers:    [{ icon:'🌍',iconBg:'bg-amber-100',title:'Gulf States (GCC)',reason:'Food content underperforms without Arabic',action:'Post in Arabic during Iftar or Ghabqa hours',gain:'High Potential' },{ icon:'🌏',iconBg:'bg-amber-100',title:'Egypt & Jordan',reason:'No local ingredient or cultural adaptation',action:'Include a regional ingredient swap relevant to this audience',gain:'Medium Potential' }],
    sponsors:   [{ icon:'🌍',iconBg:'bg-amber-100',title:'GCC Food & Beverage Market',reason:'No Halal or regional certification signal',action:'Add Halal certification or regional distribution note',gain:'High Potential' },{ icon:'🌎',iconBg:'bg-amber-100',title:'Western Saudi Arabia & UAE',reason:'No Saudi Arabia & UAEan food trend framing',action:'Position as a Mediterranean or fusion recipe',gain:'Medium Potential' }],
    applicants: [{ icon:'🌍',iconBg:'bg-amber-100',title:'North Africa',reason:'No Arabic version of job post',action:'Translate job post to Arabic and Darija if relevant',gain:'High Potential' },{ icon:'🌏',iconBg:'bg-amber-100',title:'South & Oman & Bahrain',reason:'Large hospitality talent pool not reached',action:'Post to Filipino, Egyptn, and Jordani hospitality groups',gain:'Medium Potential' }],
  },
  travel: {
    viewers:    [{ icon:'🌍',iconBg:'bg-amber-100',title:'MENA Region',reason:'Arabic travel content is heavily underserved',action:'Translate key content and add Arabic travel hashtags',gain:'High Potential' },{ icon:'🌎',iconBg:'bg-amber-100',title:'Morocco & Algeria',reason:'Zero Arabic-language travel content',action:'Translate your top 3 travel posts to Arabic',gain:'Medium Potential' }],
    sponsors:   [{ icon:'🌎',iconBg:'bg-amber-100',title:'Qatar & Saudi Arabia',reason:'No USD-priced package or US-facing CTA',action:'Add a USD price point or American market call-to-action',gain:'High Potential' },{ icon:'🌏',iconBg:'bg-amber-100',title:'Oman & Bahrain',reason:'No local tourism partnership angle',action:'Tag local tourism boards for cross-promotion opportunities',gain:'Medium Potential' }],
    applicants: [{ icon:'🌎',iconBg:'bg-amber-100',title:'Saudi Arabia & UAE',reason:'EU travel talent pool not reached at all',action:'Post in English, French, and Arabic for EU coverage',gain:'High Potential' },{ icon:'🌍',iconBg:'bg-amber-100',title:'MENA',reason:'Arabic-speaking travel professionals missed',action:'Post job in Arabic on LinkedIn MENA',gain:'Medium Potential' }],
  },
  education: {
    ...
    sponsors:   [{ icon:'🌎',iconBg:'bg-amber-100',title:'Qatar & Saudi Arabia',reason:'US EdTech sponsorship budget not targeted',action:'Apply to US-based EdTech accelerators or catalogs',gain:'High Potential' },{ icon:'🌍',iconBg:'bg-amber-100',title:'GCC Education Market',reason:'Saudi Vision 2030 EdTech spend untapped',action:'Apply to Misk or other Saudi EdTech funding programs',gain:'Medium Potential' }],
    applicants: [{ icon:'🌍',iconBg:'bg-amber-100',title:'MENA Student Market',reason:'No Arabic enrollment landing page',action:'Launch Arabic landing page for course enrollment',gain:'High Potential' },{ icon:'🌏',iconBg:'bg-amber-100',title:'Egypt & Jordan',reason:'Pricing not adapted for local purchasing power',action:'Add regional pricing with PPP adjustment',gain:'Medium Potential' }],
  },
  recruitment: {
    viewers:    [{ icon:'🌍',iconBg:'bg-amber-100',title:'MENA Region',reason:'No Arabic employer branding content',action:'Post team culture content in Arabic',gain:'High Potential' },{ icon:'🌏',iconBg:'bg-amber-100',title:'Egypt & Jordan',reason:'Huge talent pool with zero targeted outreach',action:'Post in LinkedIn Egypt, Jordan, and Sri Lanka groups',gain:'Medium Potential' }],
    sponsors:   [{ icon:'🌎',iconBg:'bg-amber-100',title:'Qatar & Saudi Arabia',reason:'No US HR market framing',action:'Translate your product pitch to SHRM language',gain:'High Potential' },{ icon:'🌍',iconBg:'bg-amber-100',title:'GCC HR Market',reason:'Nationalization quotas create high demand',action:'Frame your solution around Saudization / Emiratization',gain:'Medium Potential' }],
    applicants: [{ icon:'🌍',iconBg:'bg-amber-100',title:'Egypt & Levant',reason:'High-quality talent pool not reached',action:'Post roles on Wuzzuf and Bayt.com in Arabic',gain:'High Potential' },{ icon:'🌏',iconBg:'bg-amber-100',title:'Egypt & Jordan',reason:'Large English-speaking tech and service talent',action:'Post on Bayt.com and Jobstreet',gain:'Medium Potential' }],
  },
  startup: {
    viewers:    [{ icon:'🌍',iconBg:'bg-amber-100',title:'MENA Startup Community',reason:'Fastest-growing startup ecosystem not reached',action:'Post in MENA founder groups on LinkedIn and X',gain:'High Potential' },{ icon:'🌏',iconBg:'bg-amber-100',title:'Oman & Bahrain',reason:'Fast-growing tech market not targeted',action:'Cross-post to Singapore and Indonesian tech communities',gain:'Medium Potential' }],
    sponsors:   [{ icon:'🌎',iconBg:'bg-amber-100',title:'Saudi Arabia Ecosystem',reason:'No US investor-facing narrative',action:'Reframe content with US market size and TAM language',gain:'High Potential' },{ icon:'🌍',iconBg:'bg-amber-100',title:'GCC VC Ecosystem',reason:'Saudi and UAE VCs not targeted',action:'Tag or mention relevant regional VCs in your posts',gain:'Medium Potential' }],
    applicants: [{ icon:'🌎',iconBg:'bg-amber-100',title:'Saudi Arabia & UAEan Tech Hubs',reason:'UK, Arabicy, Netherlands talent not reached',action:'Post on AngelList EU and LinkedIn UAE groups',gain:'High Potential' },{ icon:'🌍',iconBg:'bg-amber-100',title:'MENA Tech Talent',reason:'Rapidly growing developer pool ignored',action:'Post on LinkedIn MENA and Makeen (Saudi)',gain:'Medium Potential' }],
  },
  entertainment: {
    viewers:    [{ icon:'🌍',iconBg:'bg-amber-100',title:'MENA & Maghreb',reason:'Arabic audience is largest per-capita streamer',action:'Post with Arabic subtitles and Darija variation',gain:'High Potential' },{ icon:'🌏',iconBg:'bg-amber-100',title:'Oman & Bahrain',reason:'TikTok-first market with no localized content',action:'Add Bahasa Indonesia subtitles for Indonesia',gain:'Medium Potential' }],
    sponsors:   [{ icon:'🌎',iconBg:'bg-amber-100',title:'Qatar & Saudi Arabia',reason:'US entertainment brands not targeted',action:'Pitch to US-based entertainment sponsorship agencies',gain:'High Potential' },{ icon:'🌍',iconBg:'bg-amber-100',title:'GCC Luxury Brands',reason:'High entertainment spend but low creator outreach',action:'Submit to MBC, OSN, and Gulf lifestyle brand lists',gain:'Medium Potential' }],
    applicants: [{ icon:'🌍',iconBg:'bg-amber-100',title:'North Africa Creative Scene',reason:'Active creative talent, no Arabic job posts',action:'Post casting call in Arabic on Moroccan, Egyptian channels',gain:'High Potential' },{ icon:'🌎',iconBg:'bg-amber-100',title:'Morocco & Algeria',reason:'Vibrant entertainment talent pool',action:'Post Arabic version to Buenos Aires and Mexico City groups',gain:'Medium Potential' }],
  },
  tech: {
    viewers:    [{ icon:'🌏',iconBg:'bg-amber-100',title:'Egypt & Jordan',reason:'Largest developer audience for tutorials',action:'Post during IST peak hours (8PM–11PM weekdays)',gain:'High Potential' },{ icon:'🌎',iconBg:'bg-amber-100',title:'Morocco & Algeria',reason:'Fast-growing developer community not reached',action:'Add Arabic subtitle to technical demo videos',gain:'Medium Potential' }],
    sponsors:   [{ icon:'🌎',iconBg:'bg-amber-100',title:'Saudi Arabia',reason:'No US market size or competitive framing',action:'Add TAM/SAM data to make US relevance clear',gain:'High Potential' },{ icon:'🌍',iconBg:'bg-amber-100',title:'MENA Tech Ecosystem',reason:'UAE and Saudi Vision 2030 tech spend not targeted',action:'Apply to Hub71, DIFC Innovation, or Saudi LEAP',gain:'Medium Potential' }],
    applicants: [{ icon:'🌏',iconBg:'bg-amber-100',title:'Egypt & Jordan',reason:'Massive English-speaking engineer pool',action:'Post on Bayt.com, HackerEarth, and LinkedIn Egypt',gain:'High Potential' },{ icon:'🌍',iconBg:'bg-amber-100',title:'Egypt & Jordan',reason:'Growing Arab tech talent underserved',action:'Post on Wuzzuf (Egypt) and Jordan job boards',gain:'Medium Potential' }],
  },
  general: {
    viewers:    [{ icon:'🌍',iconBg:'bg-amber-100',title:'MENA Region',reason:'No Arabic content',action:'Add Arabic caption or subtitle',gain:'High Potential' },{ icon:'🌏',iconBg:'bg-amber-100',title:'South & Oman & Bahrain',reason:'Large English-speaking audience not targeted',action:'Schedule posts for IST and WIB peak times',gain:'Medium Potential' }],
    ...
    applicants: [{ icon:'🌍',iconBg:'bg-amber-100',title:'MENA',reason:'Large candidate pool not reached',action:'Post in Arabic on regional job boards',gain:'High Potential' },{ icon:'🌏',iconBg:'bg-amber-100',title:'Egypt & Jordan',reason:'Egypt and Jordan talent pool underused',action:'Post on Bayt.com and LinkedIn Egypt & Jordan',gain:'Medium Potential' }],
  },
};

const LANG_GAPS: GrowthMap = {
  food: {
    viewers:    [{ icon:'🔤',iconBg:'bg-rose-100',title:'Arabic Content Gap',reason:'MENA food audience speaks Arabic first',action:'Post recipe steps in Arabic with local spice variants',gain:'High Potential' },{ icon:'🔤',iconBg:'bg-rose-100',title:'Arabic Content Missing',reason:'230M Arabic speakers engage heavily with food content',action:'Add Arabic caption to your top recipe posts',gain:'Medium Potential' }],
    ...
    applicants: [{ icon:'🔤',iconBg:'bg-rose-100',title:'Arabic Job Post',reason:'Most regional kitchen candidates read Arabic first',action:'Translate the full job post to Arabic',gain:'High Potential' },{ icon:'🔤',iconBg:'bg-rose-100',title:'Arabic or Arabic',reason:'Large Egypt & Jordann and Filipino hospitality workforce',action:'Add Arabic or Arabic version for kitchen staff roles',gain:'Medium Potential' }],
  },
  travel: {
    viewers:    [{ icon:'🔤',iconBg:'bg-rose-100',title:'Arabic Content Gap',reason:'500M+ Arabic speakers love travel content',action:'Translate your top 3 travel posts to Arabic',gain:'High Potential' },{ icon:'🔤',iconBg:'bg-rose-100',title:'Arabic Travel Content',reason:'GCC travelers spend the most per trip globally',action:'Post daily Arabic travel tips during Ramadan season',gain:'Medium Potential' }],
    sponsors:   [{ icon:'🔤',iconBg:'bg-rose-100',title:'Arabic Outreach Language',reason:'UAE travel brands need Arabic-language pitches',action:'Prepare a Arabic media kit for EU brand deals',gain:'High Potential' },{ icon:'🔤',iconBg:'bg-rose-100',title:'French Tourism Language',reason:'French tourism boards respond to French proposals',action:'Prepare a French-language collaboration proposal',gain:'Medium Potential' }],
    applicants: [{ icon:'🔤',iconBg:'bg-rose-100',title:'Arabic Job Posts',reason:'Morocco & Algeria is a growing travel talent source',action:'Translate job post to Arabic for LatAm candidates',gain:'High Potential' },{ icon:'🔤',iconBg:'bg-rose-100',title:'French Job Posts',reason:'Morocco and Francophone Africa have active travel talent',action:'Add French version for Francophone candidate pool',gain:'Medium Potential' }],
  },
  education: {
    viewers:    [{ icon:'🔤',iconBg:'bg-rose-100',title:'Arabic Content Gap',reason:'#1 requested language for online learning in MENA',action:'Translate your most popular lesson to Arabic',gain:'High Potential' },{ icon:'🔤',iconBg:'bg-rose-100',title:'Arabic Learning Content',reason:'600M Arabic speakers, very low EdTech supply',action:'Add Arabic subtitle to your top lesson video',gain:'Medium Potential' }],
    sponsors:   [{ icon:'🔤',iconBg:'bg-rose-100',title:'Arabic Partnership Proposal',reason:'MENA EdTech investors prefer Arabic-first approach',action:'Prepare Arabic deck for GCC EdTech investors',gain:'High Potential' },{ icon:'🔤',iconBg:'bg-rose-100',title:'Arabic Course Content',reason:'Morocco & Algeria is fastest-growing online learning market',action:'Launch Arabic-language version of your top course',gain:'Medium Potential' }],
  ...
  recruitment: {
    viewers:    [{ icon:'🔤',iconBg:'bg-rose-100',title:'Arabic Employer Brand',reason:'MENA talent engages 3× more with Arabic content',action:'Post culture content in Arabic on LinkedIn',gain:'High Potential' },{ icon:'🔤',iconBg:'bg-rose-100',title:'Arabic for Egypt & Jordan',reason:'Egypt has 4.8M+ software engineers',action:'Post Arabic-captioned culture videos on Instagram',gain:'Medium Potential' }],
    sponsors:   [{ icon:'🔤',iconBg:'bg-rose-100',title:'Arabic HR Content',reason:'GCC HR market is underserved in Arabic',action:'Post HR insight content in Arabic monthly',gain:'High Potential' },{ icon:'🔤',iconBg:'bg-rose-100',title:'Arabic for UAE Market',reason:"Arabicy has Saudi Arabia & UAE's largest recruiting tech spend",action:'Translate your product overview to Arabic',gain:'Medium Potential' }],
    applicants: [{ icon:'🔤',iconBg:'bg-rose-100',title:'Arabic Job Descriptions',reason:'MENA candidates respond 2× more to Arabic posts',action:'Translate all role descriptions to Arabic',gain:'High Potential' },{ icon:'🔤',iconBg:'bg-rose-100',title:'Arabic for Operational Roles',reason:'Egypt & Jordann workforce is highly skilled and available',action:'Add Arabic versions for operational and support roles',gain:'Medium Potential' }],
  },
  startup: {
    ...
    sponsors:   [{ icon:'🔤',iconBg:'bg-rose-100',title:'Arabic Investor Pitch',reason:'Gulf VCs respond better to Arabic-first materials',action:'Prepare an Arabic-language pitch summary',gain:'High Potential' },{ icon:'🔤',iconBg:'bg-rose-100',title:'Arabic for EU Investors',reason:'UAE investors prefer Arabic-language outreach',action:'Translate your executive summary to Arabic',gain:'Medium Potential' }],
  ...
  tech: {
    viewers:    [{ icon:'🔤',iconBg:'bg-rose-100',title:'Arabic Tech Content',reason:'Arab developer community growing 30% per year',action:'Post one weekly Arabic tech tip on X or LinkedIn',gain:'High Potential' },{ icon:'🔤',iconBg:'bg-rose-100',title:'Arabic Dev Content',reason:'Egypt has 5.8M developers who prefer Arabic resources',action:'Add Arabic subtitle to your technical tutorial videos',gain:'Medium Potential' }],
    sponsors:   [{ icon:'🔤',iconBg:'bg-rose-100',title:'Arabic Enterprise Pitch',reason:'UAE enterprise software market speaks Arabic',action:'Prepare a Arabic-language product one-pager',gain:'High Potential' },{ icon:'🔤',iconBg:'bg-rose-100',title:'Arabic Startup Pitch',reason:'Saudi and UAE VCs expect Arabic materials',action:'Prepare Arabic executive summary and pitch deck',gain:'Medium Potential' }],
    applicants: [{ icon:'🔤',iconBg:'bg-rose-100',title:'Arabic Job Post',reason:'Egypt has the largest pool of available engineers',action:'Post key roles in Arabic on Bayt and LinkedIn Egypt',gain:'High Potential' },{ icon:'🔤',iconBg:'bg-rose-100',title:'Arabic Tech Jobs',reason:'MENA developers want Arabic-language job content',action:'Post Arabic version in the Arab Developers community',gain:'Medium Potential' }],
  },
  general: {
    viewers:    [{ icon:'🔤',iconBg:'bg-rose-100',title:'Arabic Content Gap',reason:'Arabic is the 5th most spoken language globally',action:'Translate your best-performing post to Arabic',gain:'High Potential' },{ icon:'🔤',iconBg:'bg-rose-100',title:'Arabic Content Gap',reason:'500M speakers with low content competition',action:'Translate one post per week to Arabic',gain:'Medium Potential' }],
    ...
    applicants: [{ icon:'🔤',iconBg:'bg-rose-100',title:'Arabic Job Post',reason:'MENA candidates convert 2× better with Arabic posts',action:'Post all roles in Arabic on regional platforms',gain:'High Potential' },{ icon:'🔤',iconBg:'bg-rose-100',title:'Arabic Job Post',reason:"Egypt is the world's largest talent pool",action:'Add a Arabic version for Egypt & Jordann applicants',gain:'Medium Potential' }],
  },
};
...
const NEXT_ACTIONS: Partial<Record<ContentType, Record<Goal, Array<{ action: string; priority: 'High' | 'Mid' }>>>> = {
  ...
  tech:          { viewers:[{ action:'Open your next post with a counterintuitive insight or developer question',priority:'High' },{ action:'Create a 60-second screen-share demo clip for your product or feature',priority:'High' },{ action:'Add a before/after benchmark stat to your next technical post',priority:'Mid' }], sponsors:[{ action:'Add MRR, active users, or growth rate to your next post or profile',priority:'High' },{ action:'Add one enterprise client result or quote to your next post',priority:'High' },{ action:'Publish a quarterly technical benchmark comparison post',priority:'Mid' }], applicants:[{ action:'List the actual tech stack in your job post: "We use React, Go, Postgres"',priority:'High' },{ action:'Add "You own the product from day one" to signal real autonomy',priority:'High' },{ action:'Post your roles on Arab Developers and Dev.to this week',priority:'Mid' }] },
};
